    ...
    proxy.on('proxyReq', async function (p, req, res) {
      ...
      if(req.url.includes(".well-known")){
        ...
        if (xtoken) {
          ...
          try {
            userId = decode(xtoken).sub;
          } catch(err) {
            console.log('Invalid token', err);
            res.writeHead(302, {
              Location: `${domain ? domain : 'https://app.instasafe.io'}/#/login?redirect=${req.headers.host}${req.url}`
            });
            res.end();
          }
          ...
          if (isVerified) {
            ...
            verifyToken(xtoken, domain).then(tokenVerifyRes => {
...
var respondNotFound = function (req, res) {
  ...
  // res.end();
  res.writeHead(302, {
    Location: 'https://app.instasafe.io/#/404'
  });
  res.end();
};
