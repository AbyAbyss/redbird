...
function WhoIsAccessingCache() {
  ...
  this.addNew = (xtoken, domain, userId) => {
    this.userDetailList.push({xtoken, domain, userId})
  }
  this.getBasedOnUserId = (userId) => this.userDetailList.filter(e => e.userId === userId)
  this.getBasedOnXToken = (xtoken) => this.userDetailList.filter(e => e.xtoken === xtoken)
}
...
function ReverseProxy(opts) {
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    proxy.on('proxyReq', async function (p, req, res) {
      ...
      // if (!docAb) {
        ...
        // let userId = "UserId"; 
      const {[InfoEnum.XTOKENKEY]: xtoken, [InfoEnum.DOMAINKEY]: domain, [InfoEnum.USERID]: userId} = parseCookies(req.headers.cookie);
  ...
  //       var xTokenKey = 'XToken';
  //       var domainKey = "Domain";
  //       const {[xTokenKey]: xTokenValue, [domainKey]: domainUrlValue} = parseCookies(req.headers.cookie);

  //       try {
  //         if(xTokenValue){
  //           // jwt token will be in db 
  //           // if jwt token exists in memory, then only allow it,  if not redirect.
  //           // const doc = await findOne({ xToken: xTokenValue });
  //           // db.findOne({ xToken: xTokenValue }, function (err, doc) {
  //             console.log('docAb', docAb)
  //             if(true!=null){
  ...
  function redirectIOnTest(req, res) {
    ...
    // let domainKey = "Domain";
    // let userId = "UserId"; 
    const {[InfoEnum.XTOKENKEY]: xTokenValue, [InfoEnum.DOMAINKEY]: domainUrlValue, [InfoEnum.USERID]: userId} = parseCookies(req.headers.cookie);
    ...
    try {
      if(xTokenValue){
        ...
        // const doc = await findOne({ xToken: xTokenValue });
        // db.findOne({ xToken: xTokenValue }, function (err, doc) {
          console.log('docAb', whoIsAccessingCache);
          if(whoIsAccessingCache.getBasedOnXToken(xTokenValue).length) {
          //   // token exists, proceed as it is.
          }
          else{
            if(domainUrlValue && domainUrlValue.endsWith("instasafe.io")){
