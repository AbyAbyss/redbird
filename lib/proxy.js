...
var http = require('http'),
  ...
  // var db = new Datastore({ filename: OS.homedir()+'/.sdp/tokenstore.db', autoload: true });
...
const verifyTokenUrl = '/console/auth/signin/users/challenge/token';

function WhoIsAccessingCache() {
  this.userDetailList = [];
  this.userDetail = {};
  this.addNew = (xtoken, domain, userId) => {
		if (userId in this.userDetail) {
			if (xtoken === this.userDetail[userId].xtoken) {
        console.log('all fine')
      } else {
        console.log('Token might have changed');
        this.userDetail[userId] = {xtoken, domain, userId, tokenVerified: false}
      }
		} else {
      console.log('new')
      this.userDetail[userId] = {xtoken, domain, userId, tokenVerified: false}
  ...
  this.tokenVerified = (userId) => {
    this.userDetail[userId].tokenVerified = true;
  }
  this.getBasedOnUserId = (userId) => this.userDetail[userId] || null
}
const whoIsAccessingCache = new WhoIsAccessingCache()

function verifyToken(token, domain) {
  return new Promise((resolve, reject) => {
    const options = {
      method: "GET",
      headers: { "X-Token": token, 'Content-Type': 'application/json' },
      url: `${domain}${verifyTokenUrl}`
    };
...
function ReverseProxy(opts) {
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    var docAb = null
    proxy.on('proxyReq', async function (p, req, res) {
      p.socket.pause();
      if (req.host != null) {
        p.setHeader('host', req.host);
      }
      // docAb = await findOne({ userId: 'xTokenValue' });
      // if (!docAb) {
        // let xTokenKey = 'XToken';
        // let domainKey = "Domain";
        // let userId = "UserId"; 
	    
      var letsEncryptSession=false;
	    //req.
	 ...
	 p.socket.resume();
      ...
      const {[InfoEnum.XTOKENKEY]: xtoken, [InfoEnum.DOMAINKEY]: domain} = parseCookies(req.headers.cookie);

      if (xtoken) {
        const userId = decode(xtoken).sub;
  
        whoIsAccessingCache.addNew(xtoken, domain, userId);
          
        // }
        const isVerified = whoIsAccessingCache.getBasedOnUserId(userId).tokenVerified;
  
        if (isVerified) {
          redirectIOnTest(req, res)
          p.socket.resume();
        } else {
          verifyToken(xtoken, domain, userId).then(tokenVerifyRes => {
            if (tokenVerifyRes.data.success) {
              console.log('Token verified');
              whoIsAccessingCache.tokenVerified(userId);
              redirectIOnTest(req, res)
              p.socket.resume();
            } else {
              console.log('Failed to verify token', tokenVerifyRes.data.errorData);
              res.writeHead(500, {
                'Content-Type': 'text/plain'
              });
              res.end('Something went wrong, ' + tokenVerifyRes.data.errorData + '\nErrorCode: ' + tokenVerifyRes.data.errorCode + ' SubErrorCode: ' + '\nErrorCode: ' + tokenVerifyRes.data.errorSubCode);
            }
          }).catch(err => {
            console.log('Failed to verify token catch', err);
            res.writeHead(500, {
              'Content-Type': 'text/plain'
            });
        ...
        res.end('Something went wrong, ' + 'Unknown Session');
      }
    });
    }
    //
  ...
  function redirectIOnTest(req, res) {
    const {[InfoEnum.XTOKENKEY]: xTokenValue, [InfoEnum.DOMAINKEY]: domainUrlValue} = parseCookies(req.headers.cookie);
    const userId = decode(xTokenValue).sub;

    try {
      if(xTokenValue){
          const sessionInfo = whoIsAccessingCache.getBasedOnUserId(userId)
          if(sessionInfo && sessionInfo.tokenVerified) {
          //   // token exists, proceed as it is.
          }
          else{
            if(domainUrlValue && domainUrlValue.endsWith("instasafe.io")){
              res.writeHead(302, {
                Location: domainUrlValue
              });
              res.end();
            }
            else{
              res.writeHead(302, {
                Location: 'https://app.instasafe.io'
              });
              res.end();
            }
          }
  ...
  function parseCookies (cookie) {
    ...
    rc && rc.split(';').forEach(function( _cookie ) {
