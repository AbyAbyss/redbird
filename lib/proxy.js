...
var http = require('http'),
  ...
  var db = new Datastore({ filename: '/opt/instasafe/aglgateway/tokenstore.db', autoload: true });
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    proxy.on('proxyRes', function (proxyRes, req, res) {
      ...
      proxyRes.on('end', function () {
        ...
        if(req.headers[xTokenKey]!=null){
          // jwt token will be in db 
          // if jwt token exists in memory, then only allow it,  if not redirect.
          db.findOne({ xToken: req.headers[xTokenKey] }, function (err, doc) {
            if(doc!=null){
              // token exists, proceed as it is.
            }
            else{
              if(req.headers[domainKey].endsWith("instasafe.io")){
                res.redirect(req.headers[domainKey]);
              }
              else{
                res.redirect("https://app.instasafe.io");
              }
            }
          });
        }
